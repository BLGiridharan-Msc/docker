# Docker image to use. This is the base OCI image used for ths docker file execution.
FROM sloopstash/amazon-linux-2:v1.1.1

# Install the system packages and dependencies for Redis Database server.
RUN yum install tcl
-y

# Download Redis from Official Website.
RUN wget http://download.redis.io/releases/redis-7.2.1.tar.gz -P /tmp

# Extract Redis from archive.
RUN tar xvzf /tmp/redis-7.2.1.tar.gz -C /tmp

# Compile Redis from source code.
WORKDIR /tmp/redis-7.2.1
RUN set -x \
	&& make distclean \
	&& make \
	&& make install		


	sudo docker image build -t test/redis:v7.2.1 -f redis.dockerfile ./
 